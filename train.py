...
def train_lm(path, filename, epochs=8, lr=1e-3, basic=False):
    ...
    if basic:
        ...
        learn = language_model_learner(data_lm, arch=AWD_LSTM, pretrained_fnames=['lstm_wt103', 'itos_wt103'], drop_mult=0.5)
...
def train(gen, disc, epochs, trn_dl, val_dl, optimizerD, optimizerG, senti_disc=None):
    gen_iterations = 0
...
if __name__ == '__main__':
    ...
    if args.train_gan:
        ...
        senti_disc = sentiment_loss(data_lm, args.negative)

        disc.train()
        generator.train()

        train(generator, disc, args.gan_epoch, trn_dl, val_dl, optimizerD, optimizerG, senti_disc=senti_disc)
